...
#define DEBUG_PRINT(...) do { if (ENABLE_DEBUG) Serial.print(__VA_ARGS__); } while (0)
#define DEBUG_PRINTLN(...) do { if (ENABLE_DEBUG) Serial.println(__VA_ARGS__); } while (0)
#define DEBUG_PRINTF(...) do { if (ENABLE_DEBUG) Serial.printf(__VA_ARGS__); } while (0)
...
void widgetRSSFeeds::process_article_data(bool success, const String& response) {
    ...
    enum class ParseState { NONE, ITEM, TITLE, PUBDATE, CREATOR, CATEGORY, DESCRIPTION };
    ...
    while (article_count < 15 && pos < response_str.length()) {
        ...
        if (tag == "item" || tag == "entry") {
            ...
            if (state != ParseState::ITEM && state != ParseState::NONE) {
                ...
                if (content_end == std::string::npos) content_end = response_str.length();
            ...
            if (closing_tag == "item" || closing_tag == "entry") {
                ...
                if (!temp_article.headline.empty()) {
                    ...
                    DEBUG_PRINTF("DEBUG: Added article #%d: headline=%s, date=%s, creator=%s, subject=%s, comments=%s\n",
                                  article_count, temp_article.headline.c_str(), temp_article.date.c_str(),
                                  temp_article.creator.c_str(), temp_article.subject.c_str(), temp_article.comments.c_str());
                } else {
...
bool widgetRSSFeeds::redraw(uint8_t fade_amount, int8_t tab_group) {
    ...
    if (is_dirty && !was_dirty) was_dirty = true;
...
unsigned long widgetRSSFeeds::parse_date(const std::string& date_str) {
    ...
    if (date_str.length() >= 19 && date_str[4] == '-' && date_str[7] == '-' && date_str[10] == 'T') {
        ...
        valid = (year >= 1970 && month >= 1 && month <= 12 && day >= 1 && day <= 31 &&
