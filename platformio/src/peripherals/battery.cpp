...
bool Battery::init()
{
	...
	Serial.println(F("Starting MAX17048"));
	...
	Serial.print(maxlipo.getThreshold());
	Serial.println(F("%"));

	// Enable the State Of Change alert
	// SEON
	// Serial.print(F("Enabling the State Of Change alert: "));
...
void Battery::update_stats(bool forced)
{
	...
	if (millis() - next_battery_timeupdate > 5000)
	{
		...
		if ((squixl.vbus_present() == false) && (cached_rate <= 0))
		{
			...
			// squixl.log_system_message("Records: " + String(rate_historyrecords));
		}
	}
}
...
{
	Serial.print("BAT Clear Alert Status Bit");
